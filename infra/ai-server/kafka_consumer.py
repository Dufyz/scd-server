...
def process_message(consumer, producer, msg):
    ...
    Expected message format (from message.created topic):
    {
        "id": 123,
        "chat_id": 456,
        "message": "Hello world",
    ...
    try:
        ...
        if not text:
            logger.warning(f"Empty message text for ID: {message_data.get('id')}")
            return
        ...
        producer.produce(
            topic=topic_produce,
            key=str(message_data.get('id')).encode('utf-8'),
            value=json.dumps(result).encode('utf-8'),
            callback=delivery_callback
        )
        producer.poll(0)  # Trigger delivery callbacks

        logger.info(f"Language detected for message {message_data.get('id')}: {detected_language}")
